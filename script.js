...
// script.js — Versión unificada y corregida
// ============================================

// ⚠️ IMPORTANTE: Cambiá la contraseña antes de publicar.
// Para obtener un hash nuevo, abrí la consola del navegador y ejecutá:
//   hashPassword("TuNuevaContraseña").then(h => console.log(h))
// Luego reemplazá el valor de ADMIN_PASSWORD_HASH con el resultado.
...
// SheetJS (~1.1MB) y PapaParse se cargan solo
// cuando el administrador los necesita.
// ============================================
...
function calcularDescuentos(total) {
    ...
    // Envío gratis (se acumula al descuento que haya)
...
function actualizarBannerPrimeraCompra(totalCarrito) {
    ...
    // Ocultar si ya compró o la promo no está activa
...
function calcularPrecio(producto, cant = 1) {
    // ¿Está en oferta vigente?
    const hoy = new Date();
    hoy.setHours(0, 0, 0, 0);
    const oferta = ofertas.find(o => {
        if (o.productoId !== producto.id || !o.activa) return false;
        const inicio = new Date(o.fechaInicio); inicio.setHours(0, 0, 0, 0);
        const fin    = new Date(o.fechaFin);    fin.setHours(23, 59, 59, 999);
    ...
    if (sf && config.margenesSubfamilias[sf] != null) {
        margen = config.margenesSubfamilias[sf];
    } else if (fa && config.margenesFamilias[fa] != null) {
        margen = config.margenesFamilias[fa];
    } else if (se && config.margenesSecciones[se] != null) {
        margen = config.margenesSecciones[se];
    }
...
function renderOfertasDestacadas() {
    ...
    grid.innerHTML = activas.map(o => {
        ...
        const ahorro  = Math.round(((pOrig - o.precioOferta) / pOrig) * 100);
            ...
            <div class="oferta-destacada-card" onclick="agregarAlCarrito('${escapeHTMLAttr(p.id)}')">
                ...
                <div class="oferta-destacada-info">
                    ...
                    <div class="oferta-destacada-precios">
                        <span class="oferta-destacada-precio-original">$${pOrig.toLocaleString('es-AR')}</span>
                        <span class="oferta-destacada-precio-oferta">$${o.precioOferta.toLocaleString('es-AR')}</span>
                    </div>
                    <div class="oferta-destacada-ahorro">💰 Ahorrás $${(pOrig - o.precioOferta).toLocaleString('es-AR')}</div>
...
function renderProductos() {
    ...
    if (prods.length === 0) {
        grid.innerHTML = `<div class="no-results"><h3>😕 No hay productos</h3><p>Probá con otro filtro o búsqueda</p></div>`;
    ...
    grid.innerHTML = prods.map(p => {
        ...
        // Imagen con soporte WebP y fallback
        const imgHTML = validarURL(p.imagen)
            ? `<picture>
...
function actualizarCarrito() {
    ...
    // Actualizar textos del header y botón flotante
    ...
    document.getElementById('cartText').textContent = totalItems > 0
        ? `Carrito (${totalItems}) - ${montoFmt}` : 'Carrito (0)';
...
function vaciarCarrito() {
    ...
    if (confirm(`¿Vaciar el carrito? (${carrito.length} producto${carrito.length > 1 ? 's' : ''})`)) {
        carrito = [];
        guardar();
        actualizarCarrito();
...
function generarWhatsApp() {
    ...
    msg += `⏰ *Horario preferido:*\n`;
    msg += `\n_(completá estos datos antes de enviar)_`;
...
function actualizarFiltros() {
    ...
    let familias = [...new Set(
        ...
        .map(p => p.familia).filter(Boolean)
    )].sort();

    let subfamilias = [...new Set(
        (filtros.familia ? vis.filter(p => p.familia === filtros.familia)
         : filtros.seccion ? vis.filter(p => p.seccion === filtros.seccion) : vis)
        .map(p => p.subfamilia).filter(Boolean)
    )].sort();
...
function agregarProductoManual() {
    ...
    productos.push({
        ...
        costo, precio, stock: Math.max(0, stock),
        venta_diaria: 0, imagen, emoji
    });
...
function _agregarMargenCategoria(inputNombre, inputMargen, tipoConfig, label) {
    ...
    config[tipoConfig][nombre] = margen;
    guardar();
    renderMargenes();
    renderProductos();
...
// ============================================
// ADMIN — CATÁLOGO INTELIGENTE (TOP)
// ============================================
function guardarTopDinamico() {
    ...
    document.getElementById('topValorDisplay').textContent = val;
    guardar();
    renderProductos();
...
function actualizarPreviewTop() {
    const val = parseInt(document.getElementById('topExacto')?.value) || config.topDinamico;
    const prods = productos.filter(p => p.stock >= (config.stockMinimo || 0));
    prods.sort((a, b) => (b.venta_diaria || 0) - (a.venta_diaria || 0));
    const visible = prods.slice(0, val);
    const el = document.getElementById('statsDinamicas');
    if (el) el.innerHTML = `Mostrando <strong>${visible.length}</strong> de ${prods.length} productos disponibles`;
}
...
function guardarConfiguracionStockCompleta() {
    ...
    if (stockCritico >= stockBajo) return toast('⚠ Stock crítico debe ser menor que stock bajo', 'warning');
...
function actualizarPreviewStock() {
    const minimo = parseInt(document.getElementById('stockMinimoExacto')?.value) || config.stockMinimo;
    ...
    cont.innerHTML = `
        <div style="background:#f8f9fa;padding:15px;border-radius:8px;border:2px solid var(--primary);">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px;">
                ...
                <div class="form-group">
                    <label>Desde</label>
                    <input type="date" id="ofertaInicio" value="${hoy}">
                </div>
                <div class="form-group">
                    <label>Hasta</label>
                    <input type="date" id="ofertaFin" value="${en7}">
                </div>
            </div>
...
function guardarOferta(productoId) {
    ...
    // Desactivar oferta anterior del mismo producto si existe
...
function guardarConfiguracionPromos() {
    ...
    dias.forEach(({ key, id }) => {
        const activo     = document.getElementById(`promo${id}`)?.checked ?? false;
        const minimo     = parseFloat(document.getElementById(`promo${id}Min`)?.value) || 0;
        const porcentaje = parseFloat(document.getElementById(`promo${id}Desc`)?.value) || 0;
        PROMOS.dia[key]  = { activo, minimo, porcentaje };
    });
...
function cargarUIPromos() {
    ...
    const eg = document.getElementById('envioGratisActivo');
    const em = document.getElementById('envioGratisMin');
    if (eg) eg.checked = PROMOS.envioGratis.activo;
    if (em) em.value   = PROMOS.envioGratis.minimo;

    const pa = document.getElementById('primeraCompraActivo');
    const pm = document.getElementById('primeraCompraMin');
    const pd = document.getElementById('primeraCompraDesc');
    const pe = document.getElementById('primeraCompraEnvio');
    if (pa) pa.checked = PROMOS.primeraCompra.activo;
    if (pm) pm.value   = PROMOS.primeraCompra.minimo;
    if (pd) pd.value   = PROMOS.primeraCompra.porcentaje;
    if (pe) pe.checked = PROMOS.primeraCompra.incluyeEnvio;

    const ea = document.getElementById('promoEspecialActivo');
    const en = document.getElementById('promoEspecialNombre');
    const ei = document.getElementById('promoEspecialInicio');
    const ef = document.getElementById('promoEspecialFin');
    const emi= document.getElementById('promoEspecialMin');
    const ede= document.getElementById('promoEspecialDesc');
    if (ea)  ea.checked = PROMOS.especial.activo;
    if (en)  en.value   = PROMOS.especial.nombre;
    if (ei)  ei.value   = PROMOS.especial.inicio;
    if (ef)  ef.value   = PROMOS.especial.fin;
    if (emi) emi.value  = PROMOS.especial.minimo;
    if (ede) ede.value  = PROMOS.especial.porcentaje;
}

function mostrarPromosActivasHoy() {
    const cont = document.getElementById('promosActivasHoy');
    if (!cont) return;
    const hoy = new Date();
    const msgs = [];
...
async function importarExcel(e) {
    ...
    try {
        ...
        toast('❌ No se pudo cargar la librería de importación', 'error');
        return;
    }
        ...
        try {
            ...
            json.forEach(fila => {
                try {
                    ...
                    if (isNaN(costo) || costo <= 0) { errores++; return; }

                    const seccion    = (fila['Des. Seccion*'] || fila.seccion    || fila.Seccion    || 'OTROS').toString().trim().toUpperCase();
                    const familia    = (fila['Des.Grp. Familia*'] || fila.familia    || fila.Familia    || '').toString().trim().toUpperCase();
                    const subfamilia = (fila.subfamilia || fila.Subfamilia || '').toString().trim().toUpperCase();
...
async function cargarDesdeGoogleSheets(silencioso = false) {
    try {
        ...
        if (!silencioso) toast('❌ No se pudo cargar PapaParse', 'error');
        return;
    }
    ...
    try {
        ...
        for (const fila of result.data) {
            ...
            const nombre = (fila.descripcion || fila.Descripcion || fila.nombre || fila.Nombre || fila.PRODUCTO || '').trim();
            if (!nombre) continue;
...
async function verificarPassword() {
    ...
    if (hash === ADMIN_PASSWORD_HASH) {
        ...
        // Inicializar UI del admin al abrir
        renderProductosAdmin();
        renderMargenes();
        renderListaOfertas();
        cargarUIPromos();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Seguridad: asegurarse de que el panel admin y el carrito arranquen cerrados
    document.getElementById('adminPanel')?.classList.remove('open');
    document.getElementById('cartSidebar')?.classList.remove('open');
    document.getElementById('passwordModal')?.classList.remove('show');

    // Limpiar SW caché viejo si hay nueva versión disponible
    ...
    // Resincronizar productos si hace más de 6 horas
    const ultimaSync = localStorage.getItem('ultimaActualizacion');
    const hace6hs = Date.now() - 6 * 3600 * 1000;
    if (!ultimaSync || new Date(ultimaSync).getTime() < hace6hs) {
    ...
    // Event delegation para botones del carrito (qty y remove)
    ...
    // Event delegation para "Sumar al pedido" en el grid
    document.getElementById('productsGrid').addEventListener('click', e => {
        const btn = e.target.closest('.add-to-cart-btn');
        if (btn && !btn.disabled) agregarAlCarrito(btn.dataset.id);
    });

    // ── Admin: acceso triple-clic en logo ──
    ...
    document.getElementById('deleteAllBtn').addEventListener('click', () => {
        ...
        if (!confirm(`¿Eliminar los ${productos.length} productos?`)) return;
        productos = []; carrito = [];
    ...
    // ── Admin: márgenes (cargar valores actuales) ──
