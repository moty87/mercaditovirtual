...
const APPS_SCRIPT_URL  = 'https://script.google.com/macros/s/AKfycbxukp1LdFKa3u0J3POPLCyLImssgfPFsnMqVnYxQsmYYbaK6Fx-obmjjhhdCXnTbPfQyw/exec';
...
function activarSeccion(key) {
    ...
    // Scroll al catálogo con offset para compensar menús sticky
...
let zonasEnvio    = JSON.parse(localStorage.getItem('zonasEnvio')) || {
    precioBase:   1000,  // Precio fijo primeros 3km
    kmBase:       3,     // Km incluidos en precio base
    precioPorKm:  300,   // Precio por cada km adicional
    departamentos: ['guaymallen', 'maipú', 'maipu'], // Departamentos habilitados
};
// Estado de envío del cliente actual (se calcula al verificar dirección)
...
function abrirCobertura() {
    ...
    // Mostrar info de precio dinámico
...
function iniciarMapa() {
    // Siempre reiniciar para tomar coordenadas actualizadas
    if (_coberturaMap) {
    ...
    // Marcador central — punto de despacho
    L.marker([COBERTURA_LAT, COBERTURA_LNG], {
      ...
      .bindPopup('<strong>📦 Mercadito Virtual</strong><br>Punto de despacho');

    // Cursor de mano para indicar que se puede clickear
    mapEl.style.cursor = 'crosshair';

    // Click en el mapa → colocar pin del cliente
    _coberturaMap.on('click', function(e) {
        const { lat, lng } = e.latlng;
        colocarPinCliente(lat, lng);
    });

    setTimeout(() => _coberturaMap.invalidateSize(), 150);
}

// Coloca el pin del cliente en el mapa y calcula la zona
async function colocarPinCliente(lat, lng) {
    const distancia = calcularDistanciaKm(COBERTURA_LAT, COBERTURA_LNG, lat, lng);

    // Reverse geocoding para obtener el departamento
    let displayName = '';
    let departamento = '';
    try {
    ...
    } catch { /* si falla el reverse, solo usamos distancia */ }
    ...
    // Actualizar o crear marcador cliente
    ...
    _marcadorCliente = L.marker([lat, lng], {
        ...
        draggable: true  // El cliente puede arrastrar el pin para ajustar
    }).addTo(_coberturaMap);

    // Al arrastrar el pin, recalcular
        ...
        if (resultado) {
            resultado.innerHTML = `
                <div class="resultado-zona">
                    ...
                    <div><strong>Fuera de zona</strong><br><span>${motivo}</span></div>
                </div>`;
    ...
    if (resultado) {
        resultado.innerHTML = `
            <div class="resultado-zona">
                <span class="resultado-emoji">✅</span>
                <div>
                    <strong>¡Llegamos! — Envío ${costoFmt}</strong><br>
                    <span>${distTexto} km desde nuestro depósito · Arrastrá el pin para ajustar</span>
                </div>
            </div>`;
    ...
    envioCliente = {
        calculado: true,
        costo,
        distancia: parseFloat(distTexto),
        direccion: dirLabel,
        departamento,
    };
    guardar();
    ...
    try {
        ...
        if (!habilitado || !dentroRango) {
            resultado.innerHTML = `
                <div class="resultado-zona">
                    <span class="resultado-emoji">❌</span>
                    <div>
                        <strong>Fuera de zona de cobertura</strong><br>
                        <span>Por ahora solo hacemos envíos a Guaymallén y Maipú.</span>
                    </div>
                </div>`;
            resultado.className = 'cobertura-resultado resultado-error';
            resultado.style.display = 'block';
            // Limpiar envío del cliente
            envioCliente = { calculado: false, costo: 0, distancia: 0, direccion: '', departamento: '' };
            guardar();
            actualizarCarrito();
            return;
        ...
        resultado.innerHTML = `
            <div class="resultado-zona">
                ...
                <div>
                    <strong>¡Llegamos! — Envío ${costoFmt}</strong><br>
                    <span>${distTexto} km desde nuestro depósito · ${display_name.split(',')[0]}</span>
                </div>
            </div>`;
        resultado.className = 'cobertura-resultado resultado-verde';
        resultado.style.display = 'block';

        // Guardar envío calculado para el carrito
        envioCliente = {
            calculado:    true,
            costo,
            distancia:    parseFloat(distTexto),
            direccion:    display_name.split(',')[0],
            departamento: depto,
        };
        guardar();
        actualizarCarrito(); // Actualizar carrito con el costo de envío

        // Marcar en mapa usando la misma función de pin
        if (_coberturaMap) {
            _coberturaMap.setView([clienteLat, clienteLng], 14);
        }
        // colocarPinCliente maneja el marcador y el resultado
        // pero como ya tenemos todo calculado, solo colocamos el pin sin recalcular
        if (_coberturaMap) {
            if (_marcadorCliente) _marcadorCliente.remove();
...
// ============================================
// CÁLCULO DE COSTO DE ENVÍO POR KM
// ============================================
function calcularCostoEnvio(distanciaKm) {
    ...
    const kmExtra = distanciaKm - kmBase;
    return Math.round(base + kmExtra * porKm);
}
...
function esDepartamentoHabilitado(displayName) {
    // Normalizar: minúsculas + quitar acentos para comparar sin importar tildes
    const normalizar = s => s.toLowerCase()
        .normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    const dn = normalizar(displayName);
    // Nominatim devuelve "Departamento Guaymallén" o "Guaymallén"
    const habilitados = ['guaymallen', 'maipu'];
    return habilitados.some(d => dn.includes(d));
}
...
function renderProductos() {
    ...
    // En sección ofertas, agregar separador entre folleto y manuales
    ...
    grid.innerHTML = prods.map((p, idx) => {
        ...
        // Separador entre folleto y ofertas manuales
...
function filtrarProductos() {
    ...
    if (sm && sm.esOfertas) {
        // Mostrar solo productos con oferta activa (folleto o manual)
        const ofertasHoy = getOfertasActivasHoy();
        prods = prods.filter(p =>
            (p.ean && getItemFolleto(p.ean)) ||
            ofertasHoy.some(o => o.productoId === p.id)
        );
        // Ordenar: folleto primero, luego ofertas manuales
        prods.sort((a, b) => {
...
function generarWhatsApp() {
    ...
    // Agregar costo de envío calculado al mensaje
    ...
    // Si ya verificó dirección, usarla; sino la que ingresó manualmente
