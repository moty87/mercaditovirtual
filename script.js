...
// script.js — Versión mejorada v2.0
// Mejoras:
// [SEG-1] Bloqueo por intentos fallidos en admin
// [SEG-2] Salt en hash de contraseña
// [SEG-3] Rate limiting en búsqueda de ofertas
// [ROB-1] Render por lotes con requestIdleCallback (2000+ productos)
// [ROB-2] Índice de búsqueda precalculado
// [ROB-3] Gestión inteligente de cuota localStorage
// [ROB-4] Recuperación de errores en carga de Google Sheets
// [ROB-5] Carrito compartible por URL
// [ROB-6] Validación robusta del formulario de entrega
// [PERF-1] Caché de ofertas activas (no recalcula en cada render)
// [PERF-2] IntersectionObserver para imágenes
// [PERF-3] DocumentFragment en render de lotes
// [UX-1]  Banner offline/online
// [UX-2]  Animación botón "agregar al carrito"
// [UX-3]  Historial de pedidos local (últimos 10)
// [UX-4]  Placeholder con color e inicial del producto
// ============================================
...
const ADMIN_SALT          = 'mercadito_mza_v2_2025'; // [SEG-2] Salt adicional
const MAX_INTENTOS_ADMIN  = 5;                        // [SEG-1]
const BLOQUEO_MINUTOS     = 5;                        // [SEG-1]
...
const LOTE_SIZE        = 24;   // [ROB-1] Productos por lote de render
const MAX_LOCAL_PRODS  = 3 * 1024 * 1024; // 3MB límite para productos

// ============================================
// ESTADO DE SEGURIDAD ADMIN [SEG-1]
// ============================================
let intentosFallidos  = parseInt(sessionStorage.getItem('adminIntentos') || '0');
let bloqueadoHasta    = parseInt(sessionStorage.getItem('adminBloqueado') || '0');
...
// [SEG-2] Hash con salt para mayor seguridad
async function hashPassword(password) {
    const salted     = ADMIN_SALT + password + password.length;
...
// Hash legacy (para compatibilidad con contraseña actual sin salt)
...
// [PERF-1] Caché de ofertas activas (se invalida al cambiar día o al guardar)
let _ofertasCache     = null;
let _ofertasCacheDia  = -1;

// [ROB-2] Índice de búsqueda precalculado
let _indiceBusqueda = new Map();

// [PERF-2] IntersectionObserver para imágenes lazy
...
function guardar() {
    try {
        ...
        // Guardar productos con control de tamaño
        const productosJSON = JSON.stringify(productos);
        const tamañoKB = Math.round(productosJSON.length / 1024);

        if (productosJSON.length > MAX_LOCAL_PRODS) {
            console.warn(`⚠ Productos: ${tamañoKB}KB — cerca del límite de localStorage`);
            toast(`⚠ Catálogo grande (${tamañoKB}KB). Reducí el "top catálogo" en Admin.`, 'warning');
        }
        localStorage.setItem('productosTienda_v2', productosJSON);
        ...
        if (e.name === 'QuotaExceededError' || e.code === 22) {
            // Intentar guardar solo lo crítico
            try {
                localStorage.setItem('carritoTienda_v2', JSON.stringify(carrito));
                localStorage.setItem('configTienda',     JSON.stringify(config));
                toast('⚠ Almacenamiento lleno. Solo se guardó el carrito y config. Reducí el catálogo.', 'error');
            } catch {
                toast('❌ Almacenamiento crítico. Liberá espacio en el navegador.', 'error');
            }
        } else {
...
async function cargarConfigRemota() {
    try {
        ...
        if (huboCambios) {
            guardar();
            renderProductos();
            console.log('✅ Config remota aplicada');
        }
    } catch (err) {
...
// ============================================
// [PERF-1] CACHÉ DE OFERTAS ACTIVAS
// ============================================
function getOfertasActivasHoy() {
    const diaHoy = new Date().getDay();
    if (_ofertasCache !== null && _ofertasCacheDia === diaHoy) {
        return _ofertasCache;
    }

    const hoy = new Date();
    hoy.setHours(0, 0, 0, 0);

    _ofertasCache = ofertas.filter(o => {
...
function construirIndiceBusqueda() {
    _indiceBusqueda.clear();
    productos.forEach(p => {
        const palabras = p.nombre.toLowerCase().split(/\s+/);
        palabras.forEach(palabra => {
            if (palabra.length < 2) return;
            if (!_indiceBusqueda.has(palabra)) _indiceBusqueda.set(palabra, new Set());
            _indiceBusqueda.get(palabra).add(p.id);
        });
    });
}

function buscarConIndice(termino) {
    if (!termino.trim()) return null;
    const palabras = termino.toLowerCase().split(/\s+/).filter(p => p.length >= 2);
    if (!palabras.length) return null;

    let ids = null;
    palabras.forEach(palabra => {
        const coincidencias = new Set();
        _indiceBusqueda.forEach((idsSet, clave) => {
            if (clave.includes(palabra)) {
                idsSet.forEach(id => coincidencias.add(id));
            }
        });
        ids = ids === null ? coincidencias : new Set([...ids].filter(id => coincidencias.has(id)));
    });
...
// ============================================
// [UX-4] PLACEHOLDER CON COLOR E INICIAL
// ============================================
...
function generarPlaceholderSVG(producto) {
    const color = colorDesdeNombre(producto.nombre);
    const letra = producto.nombre.charAt(0).toUpperCase();
    return `data:image/svg+xml,${encodeURIComponent(
        `<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200">` +
        `<rect width="200" height="200" fill="${color}"/>` +
        `<text x="100" y="125" font-size="90" text-anchor="middle" ` +
        `font-family="sans-serif" fill="rgba(0,0,0,0.25)">${letra}</text>` +
        `</svg>`
    )}`;
}
...
// [PERF-2] INTERSECTIONOBSERVER PARA IMÁGENES
// ============================================
function iniciarImageObserver() {
    if (_imageObserver) return;
...
function observarImagenesPendientes() {
    if (!_imageObserver) return;
...
function renderOfertasDestacadas() {
    ...
    if (!activas.length) { seccion.style.display = 'none'; return; }
    ...
    grid.innerHTML = activas.map(o => {
        ...
        const srcImagen = validarURL(p.imagen) ? p.imagen : generarPlaceholderSVG(p);

        return `
            <div class="oferta-destacada-card" onclick="agregarAlCarrito('${escapeHTMLAttr(p.id)}')">
                <div class="oferta-destacada-badge">🔥 ${ahorro}% OFF</div>
                <div class="oferta-destacada-image">
                    <img data-src="${escapeHTMLAttr(srcImagen)}"
                         src="${generarPlaceholderSVG(p)}"
                         alt="${escapeHTMLAttr(p.nombre)}"
                         decoding="async"
                         onerror="this.src='${generarPlaceholderSVG(p)}'">
                </div>
    ...
    observarImagenesPendientes();
}

// ============================================
// [ROB-1] RENDER POR LOTES CON requestIdleCallback
// ============================================
let _renderCancelToken = 0; // Para cancelar renders anteriores si se llama de nuevo

function generarHTMLProducto(p, ofertasHoyCache) {
    const p1   = calcularPrecio(p, 1, ofertasHoyCache);
    const p3   = calcularPrecio(p, 3, ofertasHoyCache);
    const enOf = ofertasHoyCache.some(o => o.productoId === p.id);
    const precioOriginal = getPrecioOriginal(p);
    const ahorro = enOf ? Math.round(((precioOriginal - p1) / precioOriginal) * 100) : 0;
    ...
    const srcImagen = validarURL(p.imagen) ? p.imagen : generarPlaceholderSVG(p);
    const placeholder = generarPlaceholderSVG(p);

    const imgHTML = `<img src="${escapeHTMLAttr(placeholder)}"
                         data-src="${escapeHTMLAttr(srcImagen)}"
                         alt="${escapeHTMLAttr(p.nombre)}"
                         decoding="async"
                         onerror="this.src='${placeholder}'">`;

    const precioHTML = enOf
    ...
    return `
        <div class="product-card ${enOf ? 'en-oferta' : ''}">
            ${enOf ? `<div class="oferta-badge">🔥 ${ahorro}% OFF</div>` : ''}
    ...
    if (prods.length === 0) {
        grid.innerHTML = `
            <div class="no-results">
                ...
                ${filtros.busqueda ? `<p>Intentá con una palabra más corta</p>` : ''}
            </div>`;
        document.getElementById('relatedProducts').style.display = 'none';
        return;
    }

    // [ROB-1] Render por lotes
    grid.innerHTML = '';
    const ofertasHoyCache = getOfertasActivasHoy();
    const token = ++_renderCancelToken;
    let indice = 0;

    function renderLote() {
        // Si se inició un nuevo render, cancelar este
        if (token !== _renderCancelToken) return;

        const lote = prods.slice(indice, indice + LOTE_SIZE);
        const fragment = document.createDocumentFragment();

        lote.forEach(p => {
            const div = document.createElement('div');
            div.innerHTML = generarHTMLProducto(p, ofertasHoyCache);
        ...
        if (indice < prods.length) {
            if ('requestIdleCallback' in window) {
                requestIdleCallback(renderLote, { timeout: 400 });
            } else {
                setTimeout(renderLote, 16);
            }
        }
    }
...
function filtrarProductos() {
    ...
    if (filtros.busqueda.trim()) {
        // [ROB-2] Usar índice de búsqueda
        const ids = buscarConIndice(filtros.busqueda);
        if (ids !== null) {
...
// [UX-2] Animación del botón al agregar
...
function compartirCarrito() {
    ...
    try {
        const data    = carrito.map(i => `${i.id}:${i.cantidad}`).join(',');
        const encoded = btoa(encodeURIComponent(data));
        const url     = `${window.location.origin}${window.location.pathname}?carrito=${encoded}`;
        navigator.clipboard.writeText(url).then(() => {
            toast('🔗 Link del carrito copiado al portapapeles', 'success');
        }).catch(() => {
            // Fallback si clipboard no está disponible
            prompt('Copiá este link para compartir tu carrito:', url);
        });
    } catch (e) {
...
function restaurarCarritoDesdeURL() {
    const params       = new URLSearchParams(window.location.search);
    const carritoParam = params.get('carrito');
    if (!carritoParam) return;

    try {
        const data  = decodeURIComponent(atob(carritoParam));
        ...
        items.forEach(item => {
            ...
            const cantFinal = Math.min(cant, p.stock);
            const existente = carrito.find(x => x.id === id);
            if (existente) {
                existente.cantidad = Math.min(existente.cantidad + cantFinal, p.stock);
            } else {
                carrito.push({
                    id: p.id, nombre: p.nombre, costo: p.costo,
                    cantidad: cantFinal, imagen: p.imagen, emoji: p.emoji,
                    seccion: p.seccion, familia: p.familia, subfamilia: p.subfamilia
                });
            }
            restaurados++;
        ...
        if (restaurados > 0) {
            guardar();
            actualizarCarrito();
            toast(`🛒 Carrito restaurado (${restaurados} producto${restaurados > 1 ? 's' : ''})`, 'success');
        }

        // Limpiar URL sin recargar
        window.history.replaceState({}, '', window.location.pathname);
    } catch (e) {
        console.warn('Carrito URL inválido:', e.message);
        window.history.replaceState({}, '', window.location.pathname);
    }
}

// ============================================
// [ROB-6] VALIDACIÓN ROBUSTA DEL FORMULARIO
// ============================================
function validarFormularioEntrega() {
    ...
    if (nombre && nombre.length < 3) {
        toast('⚠ El nombre es muy corto (mínimo 3 caracteres)', 'warning');
    ...
    if (!direccion) {
        toast('⚠ Ingresá tu dirección de entrega', 'warning');
        document.getElementById('deliveryAddress').focus();
        return false;
    }
    if (direccion.length < 8) {
        toast('⚠ Dirección muy corta — incluí calle y número', 'warning');
        document.getElementById('deliveryAddress').focus();
        return false;
    }

    // Detectar direcciones demasiado genéricas
    const vagos = ['mi casa', 'acá', 'aca', 'ahí', 'ahi', 'casa', 'home'];
    if (vagos.some(v => direccion.toLowerCase().trim() === v)) {
        toast('⚠ Necesitamos la dirección completa con calle y número', 'warning');
        document.getElementById('deliveryAddress').focus();
        return false;
    }
...
// ============================================
// [UX-3] HISTORIAL DE PEDIDOS LOCAL
// ============================================
function guardarPedidoEnHistorial(total, totalFinal) {
    try {
        ...
        // Solo guardar los últimos 10 pedidos
        localStorage.setItem('historialPedidos', JSON.stringify(historial.slice(0, 10)));
    } catch (e) {
...
function generarWhatsApp() {
    if (carrito.length === 0) { toast('⚠ El carrito está vacío', 'warning'); return; }
    if (!validarFormularioEntrega()) return;  // [ROB-6]
    ...
    // [UX-3] Guardar en historial
    guardarPedidoEnHistorial(totalSinDescuento, descuentos.totalFinal || totalSinDescuento);
...
// ============================================
// [UX-1] MONITOR DE CONEXIÓN OFFLINE/ONLINE
// ============================================
function iniciarMonitorConexion() {
    ...
    banner.textContent = '📵 Sin conexión — Los precios pueden no estar actualizados';
    document.body.prepend(banner);

    window.addEventListener('offline', () => {
        banner.style.display = 'block';
        toast('📵 Sin conexión a internet', 'warning');
    });

    window.addEventListener('online', () => {
        banner.style.display = 'none';
        toast('✅ Conexión restaurada — Actualizando catálogo...', 'success');
...
function eliminarProducto(id) {
    confirmarAccion('¿Eliminar este producto?', () => {
        ...
        construirIndiceBusqueda(); // Reconstruir índice [ROB-2]
...
function agregarProductoManual() {
    ...
    productos.push({ id: generarIdUnico(), nombre, seccion, familia, subfamilia, costo, precio, stock: Math.max(0, stock), venta_diaria: 0, imagen, emoji });
    construirIndiceBusqueda(); // [ROB-2]
...
function parsearFilaProducto(fila) {
    ...
    return {
        id: generarIdUnico(), nombre, seccion, familia, subfamilia,
...
async function cargarDesdeGoogleSheets(silencioso = false) {
    ...
    const productosAnteriores = [...productos]; // Backup por si falla
    ...
    try {
        ...
        if (result.errors.length > 10) {
            console.warn(`CSV con ${result.errors.length} errores. Primeros:`, result.errors.slice(0, 3));
        }
        const nuevos = [];
        ...
        // [ROB-4] Restaurar backup si la carga falló
        if (productosAnteriores.length > 0 && productos.length === 0) {
            productos = productosAnteriores;
            console.warn('⚠ Carga fallida, restaurando productos anteriores');
        }
...
// ============================================
// [SEG-1] VERIFICACIÓN DE PASSWORD CON BLOQUEO
// ============================================
async function verificarPassword() {
    // Verificar bloqueo activo
    const ahora = Date.now();
    if (ahora < bloqueadoHasta) {
        const segsRestantes = Math.ceil((bloqueadoHasta - ahora) / 1000);
        const mins = Math.floor(segsRestantes / 60);
        const segs = segsRestantes % 60;
        toast(`🔒 Demasiados intentos. Intentá en ${mins > 0 ? mins + 'm ' : ''}${segs}s`, 'error');
        return;
    }
    ...
    // [SEG-2] Verificar con hash legacy (compatibilidad) y nuevo hash con salt
    const hashLegacy = await hashPasswordLegacy(pass);
    const hashNuevo  = await hashPassword(pass);
    const esValida   = hashLegacy === ADMIN_PASSWORD_HASH || hashNuevo === ADMIN_PASSWORD_HASH;

    if (esValida) {
        // Acceso concedido — resetear intentos
        intentosFallidos = 0;
        sessionStorage.removeItem('adminIntentos');
        sessionStorage.removeItem('adminBloqueado');
        ...
        // Acceso denegado
        intentosFallidos++;
        sessionStorage.setItem('adminIntentos', intentosFallidos.toString());

        if (intentosFallidos >= MAX_INTENTOS_ADMIN) {
            bloqueadoHasta = Date.now() + BLOQUEO_MINUTOS * 60 * 1000;
            sessionStorage.setItem('adminBloqueado', bloqueadoHasta.toString());
            toast(`🔒 Demasiados intentos. Bloqueado ${BLOQUEO_MINUTOS} minutos.`, 'error');
        } else {
            const restantes = MAX_INTENTOS_ADMIN - intentosFallidos;
            toast(`❌ Contraseña incorrecta (${restantes} intento${restantes !== 1 ? 's' : ''} restante${restantes !== 1 ? 's' : ''})`, 'error');
        }
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🔄 Mercadito Virtual v2.0 iniciando...');
    ...
    // [UX-1] Monitor offline/online
    iniciarMonitorConexion();

    // [PERF-2] Iniciar observer de imágenes
    iniciarImageObserver();

    // Cargar config remota
    cargarConfigRemota();
    ...
    if (!ultimaSync || new Date(ultimaSync).getTime() < hace6hs || productos.length === 0) {
        ...
        // Construir índice con datos locales [ROB-2]
    ...
    // [ROB-5] Restaurar carrito desde URL si viene compartido
    if (productos.length > 0) {
        restaurarCarritoDesdeURL();
    } else {
        // Esperar a que carguen los productos antes de restaurar
        ...
        setTimeout(() => clearInterval(esperarProductos), 10000);
    }
    ...
    // Grid de productos — [UX-2] animación al agregar
    document.getElementById('productsGrid').addEventListener('click', e => {
        const btn = e.target.closest('.add-to-cart-btn');
        if (btn && !btn.disabled) {
            agregarAlCarrito(btn.dataset.id);
            animarBotonCarrito(btn);
        }
    });
    ...
    // Sliders sincronizados (stock/top)
    ...
    console.log('✅ Mercadito Virtual v2.0 listo');
});
