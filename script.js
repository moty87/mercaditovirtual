...
function iniciarMapa() {
    ...
    // Solo marcador central — las zonas se determinan por departamento, no por círculo
    ...
    try {
        ...
        if (_coberturaMap) {
            ...
            _marcadorCliente = L.marker([clienteLat, clienteLng])
                .addTo(_coberturaMap)
                .bindPopup(`📍 ${display_name.split(',')[0]}<br>Envío: ${costoFmt} (${distTexto} km)`)
                .openPopup();
            _coberturaMap.setView([clienteLat, clienteLng], 13);
        }
