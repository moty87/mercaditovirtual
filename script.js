...
// Cambios principales:
// - Modal de entrega antes de abrir WhatsApp (reduce fricción)
// - Buscador mobile con toggle (nueva función)
// - Barra de progreso envío gratis en carrito
// - CSS custom properties para sticky heights calculadas por JS
// - Eliminado WebP falso generado por JS (evita 404s)
// - Función getOfertasActivasHoy() centralizada (evita duplicación)
// - confirm() reemplazado por modal nativo (compatible con PWAs)
// - Auto-sync Google Sheets al iniciar si cache es viejo
// ============================================
...
const APPS_SCRIPT_URL  = 'https://script.google.com/macros/s/AKfycbx_Nm5J0o0YZLTccrRJhBVxbLk_EhAlbdwkXRhiXz6MxQDZAFIshA9mS3Nmn7FObpDoOA/exec';
...
// Lee la config desde la hoja "config" del Sheet
// y la aplica a todos los clientes al cargar la página
...
// Escribe la config actual en la hoja "config" del Sheet
// Solo la llama el admin al guardar
async function guardarConfigRemota(configParcia) {
    try {
        const response = await fetch(APPS_SCRIPT_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(configParcia)
        });
        const result = await response.json();
        if (result.ok) {
            console.log('✅ Config guardada en Sheet');
            return true;
        } else {
        ...
        console.error('❌ Error de red al guardar config:', err.message);
        return false;
    }
}
...
// (evita duplicar la lógica de fecha en 4 lugares)
// ============================================
...
function calcularPrecio(producto, cant = 1) {
    // Usar la función centralizada de ofertas
...
function renderOfertasDestacadas() {
    ...
    grid.innerHTML = activas.map(o => {
        ...
        // FIX: sin generación de WebP falso — solo mostrar la imagen original
...
function renderProductos() {
    ...
    grid.innerHTML = prods.map(p => {
        ...
        // FIX: sin generación de WebP falso
        // Si tenés archivos .webp en tu servidor, agregá la URL directamente al campo imagen del producto
        const imgHTML = validarURL(p.imagen)
            ? `<img src="${escapeHTMLAttr(p.imagen)}" alt="${escapeHTMLAttr(p.nombre)}" loading="lazy" decoding="async"
                   onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
               <div class="image-placeholder" style="display:none;">${escapeHTML(p.emoji) || '📦'}</div>`
...
function filtrarProductos() {
    ...
    // PASO 1: Seleccionar los N más vendidos del catálogo completo
    prods.sort((a, b) => (b.venta_diaria || 0) - (a.venta_diaria || 0));
    prods = prods.slice(0, config.topDinamico || 100);

    // PASO 2: Ordenar esos N por Sección → Familia → venta_diaria
    // Así la Coca 2.5L entra al top por su venta, y dentro del top se agrupa bien
    prods.sort((a, b) => {
...
function actualizarCarrito() {
    ...
    // Detectar mobile para texto compacto del carrito en el header
    ...
    // Actualizar banner primera compra y barra envío gratis
...
// Captura dirección antes de abrir WhatsApp
// ============================================
...
// ============================================
// WHATSAPP (ahora llamado desde el modal de entrega)
// ============================================
function generarWhatsApp() {
    if (carrito.length === 0) { toast('⚠ El carrito está vacío', 'warning'); return; }

    // Leer datos del modal de entrega
    ...
    // Datos de entrega (ya completos, sin que el usuario edite en WhatsApp)
    msg += `\n━━━━━━━━━━━━━━━━━\n`;
...
function mostrarPromosActivasHoy() {
    ...
    if (PROMOS.especial?.activo && PROMOS.especial.inicio && PROMOS.especial.fin) {
        const inicio = new Date(PROMOS.especial.inicio);
        const fin    = new Date(PROMOS.especial.fin); fin.setHours(23, 59, 59, 999);
            ...
            json.forEach(fila => {
                try {
                    ...
                    nuevos.push({
                        ...
                        stock, venta_diaria, imagen: fila.imagen || fila.Imagen || '', emoji: fila.emoji || fila.Emoji || '📦'
                    });
                } catch { errores++; }
            });
...
async function verificarPassword() {
    ...
    if (hash === ADMIN_PASSWORD_HASH) {
        ...
        // Restaurar valores guardados en todos los controles del admin
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Calcular alturas sticky dinámicamente y actualizar CSS custom properties
    // Esto corrige el nav sticky cuando el badge ocupa más de una línea en móvil
    ...
    if (badge) {
        ...
        updateStickyHeights();
        // Re-calcular en resize por si el badge cambia de líneas
        window.addEventListener('resize', debounce(updateStickyHeights, 200));
    }

    // Seguridad: asegurarse de que panels arranquen cerrados
    ...
    // Cargar config remota del Sheet (para que todos los usuarios vean los mismos valores)
    cargarConfigRemota();

    // Auto-sync Google Sheets: si cache tiene más de 6 horas o no hay productos
    ...
    // ── Búsqueda mobile (toggle con ícono lupa) ──
    ...
    // Event delegation carrito (qty y remove)
    ...
    // Event delegation grid de productos
    ...
    // Enter en campos del modal dispara envío
