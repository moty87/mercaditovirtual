...
let config = JSON.parse(localStorage.getItem('configTienda')) || {
    ...
    margenesSubfamilias: {},
    topDinamico: 100,
    // NUEVAS CONFIGURACIONES DE STOCK
    stockMinimo: 20,      // Stock mínimo para mostrar (ej: 20)
    stockCritico: 5,      // Stock crítico (rojo)
    stockBajo: 20         // Stock bajo (amarillo)
};
...
// ============================================
// NUEVA FUNCIÓN: FILTRADO POR STOCK MÍNIMO
// ============================================
function filtrarPorStockMinimo(productosArray) {
    const stockMinimo = config.stockMinimo || 0;
    return productosArray.filter(p => p.stock >= stockMinimo);
}
...
function obtenerProductosVisibles() {
    // 1️⃣ PRIMERO: Filtrar por stock mínimo
    let productosConStockMinimo = filtrarPorStockMinimo(productos);
...
// ============================================
// NUEVA FUNCIÓN: ACTUALIZAR PREVIEW DE STOCK
// ============================================
function actualizarPreviewStock() {
    const stockMinimo = parseInt(document.getElementById('stockMinimoExacto').value) || 0;
    
    const totalProductos = productos.length;
    const conStockPositivo = productos.filter(p => p.stock > 0).length;
    const conStockMinimo = productos.filter(p => p.stock >= stockMinimo).length;
    const ocultos = totalProductos - conStockMinimo;
    
    const stats = document.getElementById('statsStockFiltro');
    if (stats) {
        stats.innerHTML = `
            <div style="display: grid; gap: 5px;">
                <div>📦 Total productos: <strong>${totalProductos}</strong></div>
                <div>✅ Stock > 0: <strong>${conStockPositivo}</strong></div>
                <div>🎯 Stock ≥ ${stockMinimo}: <strong style="color: var(--success);">${conStockMinimo}</strong></div>
                <div>❌ Ocultos automáticamente: <strong style="color: var(--primary);">${ocultos}</strong></div>
                <div style="border-top: 1px dashed #ccc; margin: 5px 0;"></div>
                <div>📊 ${((conStockMinimo/totalProductos)*100).toFixed(1)}% del catálogo visible</div>
            </div>
        `;
    }
    
    // Actualizar slider máximo del top dinámico
    actualizarMaximoSlider();
}

// ============================================
// NUEVA FUNCIÓN: GUARDAR CONFIGURACIÓN DE STOCK
// ============================================
function guardarConfiguracionStockCompleta() {
    ...
    // Validaciones
    if (stockMinimo < 0) {
        toast('⚠ El stock mínimo no puede ser negativo', 'warning');
        return;
    }
    
    if (stockCritico < 1) {
        toast('⚠ El stock crítico debe ser al menos 1', 'warning');
        return;
    }
    
    if (stockBajo <= stockCritico) {
        toast('⚠ El stock bajo debe ser mayor que el stock crítico', 'warning');
        return;
    }
...
// ============================================
// NUEVA FUNCIÓN: INICIALIZAR CONTROLES DE STOCK
// ============================================
function inicializarControlesStock() {
    // Stock mínimo
    const minRange = document.getElementById('stockMinimoRange');
    const minExacto = document.getElementById('stockMinimoExacto');
    
    if (minRange && minExacto) {
        minRange.value = config.stockMinimo || 20;
        minExacto.value = config.stockMinimo || 20;
        
        minRange.addEventListener('input', () => {
            minExacto.value = minRange.value;
            actualizarPreviewStock();
        });
        
        minExacto.addEventListener('input', () => {
            let val = parseInt(minExacto.value) || 0;
    ...
    // Stock crítico
    const criticoRange = document.getElementById('stockCriticoRange');
    const criticoExacto = document.getElementById('stockCriticoExacto');
    
    if (criticoRange && criticoExacto) {
        criticoRange.value = config.stockCritico || 5;
        criticoExacto.value = config.stockCritico || 5;
        
        criticoRange.addEventListener('input', () => {
            criticoExacto.value = criticoRange.value;
            document.getElementById('stockBajoMin').value = parseInt(criticoRange.value) + 1;
            document.getElementById('stockNormalMin').value = parseInt(document.getElementById('stockBajoExacto').value) + 1;
        });
        
        criticoExacto.addEventListener('input', () => {
            let val = parseInt(criticoExacto.value) || 1;
            val = Math.max(1, Math.min(50, val));
            criticoRange.value = val;
            criticoExacto.value = val;
    ...
    // Stock bajo
    const bajoRange = document.getElementById('stockBajoRange');
    const bajoExacto = document.getElementById('stockBajoExacto');
    
    if (bajoRange && bajoExacto) {
        bajoRange.value = config.stockBajo || 20;
        bajoExacto.value = config.stockBajo || 20;
        bajoRange.min = (config.stockCritico || 5) + 1;
        
        bajoRange.addEventListener('input', () => {
            bajoExacto.value = bajoRange.value;
            document.getElementById('stockNormalMin').value = parseInt(bajoRange.value) + 1;
        });
        
        bajoExacto.addEventListener('input', () => {
            let val = parseInt(bajoExacto.value) || 6;
            const min = (config.stockCritico || 5) + 1;
            val = Math.max(min, Math.min(100, val));
            bajoRange.value = val;
            bajoExacto.value = val;
...
function actualizarMaximoSlider() {
    ...
    if (slider) {
        ...
        // Ajustar valor si es mayor al máximo
...
function actualizarPreviewDinamico() {
    ...
    timeoutPreview = setTimeout(() => {
        ...
        // Usar productos con stock mínimo
...
function renderOfertasDestacadas() {
    ...
    // Las ofertas se muestran SIEMPRE, sin filtro de stock mínimo
...
function renderProductos() {
    ...
    grid.innerHTML = prods.map(p => {
        ...
        // ============================================
        // NUEVA LÓGICA DE STOCK CONFIGURABLE
        // ============================================
        let stockBadge = '';
        let stockClass = '';
        let stockText = '';
...
// Crear nombre corto (máximo 50 caracteres) para mostrar en la tarjeta
...
// ============================================
// FUNCIONES DEL ADMIN: MÁRGENES (sin cambios)
// ============================================
...
// ============================================
// FUNCIONES DEL ADMIN: IMPORTACIÓN
// ============================================
