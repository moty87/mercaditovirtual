...
// ============================================
// CONFIGURACIÓN DE DESCUENTOS Y PROMOS (AHORA VARIABLE)
// ============================================
...
// ============================================
// FUNCIONES DE PRIMERA COMPRA Y PROMOS (ACTUALIZADAS)
// ============================================
...
function getPromoDelDia() {
    ...
    const promoDia = PROMOS.dia[hoy];
    if (!promoDia || !promoDia.activo) return null;
    return promoDia;
}
...
// ============================================
// ACTUALIZAR CARRITO (con Google Analytics)
// ============================================
function actualizarCarrito() {
    ...
    // 🎯 Google Analytics - Valor del carrito
...
// ============================================
// GENERAR WHATSAPP CON DESCUENTOS Y GOOGLE ANALYTICS
// ============================================
function generarWhatsApp() {
    ...
    // 🎯 Google Analytics - Inicio de checkout
...
function agregarAlCarrito(id) {
    ...
    // 🎯 Google Analytics - Agregar al carrito (usando calcularPrecio)
    if (typeof gtag === "function") {
        const precioReal = calcularPrecio(p, 1);
...
// ============================================
// FUNCIONES DE FILTROS (ACTUALIZADAS)
// ============================================
function actualizarFiltros() {
    ...
    // Obtener familias según la sección seleccionada
    let familias = [];
    if (filtros.seccion) {
    ...
    // Obtener subfamilias según la sección y familia seleccionadas
    let subfamilias = [];
    if (filtros.familia) {
    ...
    function actualizarSelect(id, opciones, valorActual) {
        ...
        // Intentar establecer el valor que debería tener según filtros
        if (valorActual && opciones.includes(valorActual)) {
            sel.value = valorActual;
        } else {
            sel.value = ''; // Seleccionar "Todas"
        }
    }
    
    // Actualizar todos los selects (modal y desktop)
...
function verProducto(id) {
    ...
    // 🎯 Google Analytics - Vista de producto
    if (typeof gtag === "function") {
...
// ============================================
// NUEVAS FUNCIONES PARA GESTIÓN DE PROMOCIONES
// ============================================

function cargarConfigPromos() {
    const promosGuardadas = JSON.parse(localStorage.getItem('promosConfig'));
    if (!promosGuardadas) {
        const promosDefault = {
            dia: {
                1: { activo: document.getElementById('promoLunes')?.checked || true, minimo: 20000, porcentaje: 8 },
            ...
            envioGratis: { activo: true, minimo: 35000 },
        ...
        localStorage.setItem('promosConfig', JSON.stringify(promosDefault));
        return promosDefault;
    }
    return promosGuardadas;
}

function aplicarConfigPromosAInputs() {
    const promos = cargarConfigPromos();
...
function guardarConfiguracionPromos() {
    ...
    localStorage.setItem('promosConfig', JSON.stringify(promosConfig));
    PROMOS = promosConfig;
    toast('✅ Configuración de promociones guardada', 'success');
    actualizarPromosActivasHoy();
}
window.guardarConfiguracionPromos = guardarConfiguracionPromos; // Hacerla global
...
function actualizarPromosActivasHoy() {
    ...
    if (promos.dia[hoy] && promos.dia[hoy].activo) {
        html += `<div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px; border-radius: 10px; margin-bottom: 10px;">
    ...
    if (promos.envioGratis.activo) {
        html += `<div style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; padding: 15px; border-radius: 10px; margin-bottom: 10px;">
            <strong>🚚 Envío Gratis</strong><br>
            En compras > $${promos.envioGratis.minimo.toLocaleString('es-AR')}
        </div>`;
    }
    if (promos.primeraCompra.activo) {
        html += `<div style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); color: white; padding: 15px; border-radius: 10px; margin-bottom: 10px;">
    ...
    if (promos.especial && promos.especial.activo && promos.especial.inicio && promos.especial.fin) {
        ...
        if (hoyFecha >= inicio && hoyFecha <= fin) {
            html += `<div style="background: linear-gradient(135deg, #fa709a 0%, #fee140 100%); color: #333; padding: 15px; border-radius: 10px; margin-bottom: 10px;">
    ...
    if (html === '') html = '<p style="color: #666;">No hay promociones activas hoy</p>';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Inicializar promociones si existen
    if (document.getElementById('promoLunes')) {
        aplicarConfigPromosAInputs();
    }
    
    // Eventos para preview de promo especial
