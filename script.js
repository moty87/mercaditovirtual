...
function resetearPrimeraCompra() {
    usuarioYaCompro = false;
    localStorage.removeItem('yaCompro');
    toast('🔄 Modo prueba: primera compra activado', 'success');
}
...
// ============================================
// FUNCIONES DE ADMIN (MÁRGENES, STOCK, OFERTAS)
// ============================================
...
function crearOferta(id) {
    ...
    inicio.setDate(hoy.getDate() + (1 - hoy.getDay())); // Lunes
...
function importarExcel(e) {
    toast('✅ Productos importados (simulado)', 'success');
}

async function cargarDesdeGoogleSheets() {
    toast('✅ Productos cargados desde Google Sheets (simulado)', 'success');
}

function exportarProductos() {
    toast('✅ Productos exportados (simulado)', 'success');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Setear valores de configuración
    ...
    ['stockMinimoRange', 'stockCriticoRange', 'stockBajoRange'].forEach(id => {
        ...
        if (el) el.addEventListener('input', actualizarPreviewDinamico);
    });
    
    // Configurar autocompletado de ofertas
    ...
    // ✅ TABS DEL ADMIN - AHORA FUNCIONAN
    ...
    // Sincronizar selects del modal con desktop
