...
function activarSeccion(key) {
    ...
    // Resetear filtros de los selects
    filtros.seccion    = '';
    filtros.familia    = '';
    filtros.subfamilia = '';
    filtros.busqueda   = '';
...
let zonasEnvio    = JSON.parse(localStorage.getItem('zonasEnvio')) || {
    zona1: { precio: 0,    texto: 'Envío gratis' },
    zona2: { precio: 500,  texto: 'Envío $500' },
    zona3: { precio: 1000, texto: 'Envío $1.000' },
};
let ordenActual   = 'vendidos'; // vendidos | precio_asc | precio_desc | nombre_asc | favoritos
...
// ZONA DE COBERTURA
// ============================================
const COBERTURA_LAT = -32.8890;
const COBERTURA_LNG = -68.8150;
...
function abrirCobertura() {
    ...
    // Actualizar textos de zonas
    document.getElementById('zonaTexto1').textContent = zonasEnvio.zona1.texto;
    document.getElementById('zonaTexto2').textContent = zonasEnvio.zona2.texto;
    document.getElementById('zonaTexto3').textContent = zonasEnvio.zona3.texto;
...
function iniciarMapa() {
    ...
    // Círculos de zonas
    L.circle([COBERTURA_LAT, COBERTURA_LNG], {
        radius: 5000, color: '#2d6a4f', fillColor: '#52b788', fillOpacity: 0.2, weight: 2
    }).addTo(_coberturaMap).bindPopup(`🟢 Zona 1 — ${zonasEnvio.zona1.texto}`);

    L.circle([COBERTURA_LAT, COBERTURA_LNG], {
        radius: 8000, color: '#e9c46a', fillColor: '#e9c46a', fillOpacity: 0.1, weight: 2
    }).addTo(_coberturaMap).bindPopup(`🟡 Zona 2 — ${zonasEnvio.zona2.texto}`);

    L.circle([COBERTURA_LAT, COBERTURA_LNG], {
        radius: 10000, color: '#e63946', fillColor: '#e63946', fillOpacity: 0.07, weight: 2
    }).addTo(_coberturaMap).bindPopup(`🔴 Zona 3 — ${zonasEnvio.zona3.texto}`);

    // Marcador central
    L.marker([COBERTURA_LAT, COBERTURA_LNG])
        .addTo(_coberturaMap)
        .bindPopup('📦 Mercadito Virtual<br>Punto de despacho')
        .openPopup();
...
async function verificarDireccion() {
    ...
    try {
        // Geocodificar con Nominatim — agregar Mendoza Argentina para mejorar resultados
        ...
        const response = await fetch(url, {
            headers: { 'Accept-Language': 'es', 'User-Agent': 'MercaditoVirtualMendoza/1.0' }
        });
        ...
        let zona, emoji, clase, precio;
        if (distancia <= 5) {
            zona = 'Zona 1'; emoji = '🟢'; clase = 'resultado-verde';
            precio = zonasEnvio.zona1.precio === 0 ? zonasEnvio.zona1.texto : `$${zonasEnvio.zona1.precio.toLocaleString('es-AR')}`;
        } else if (distancia <= 8) {
            zona = 'Zona 2'; emoji = '🟡'; clase = 'resultado-amarillo';
            precio = zonasEnvio.zona2.texto;
        } else if (distancia <= 10) {
            zona = 'Zona 3'; emoji = '🔴'; clase = 'resultado-rojo';
            precio = zonasEnvio.zona3.texto;
        } else {
            zona = null; emoji = '❌'; clase = 'resultado-error';
        }
        ...
        if (zona) {
            resultado.innerHTML = `
                <div class="resultado-zona">
                    ...
                    <div>
                        <strong>¡Llegamos! — ${zona}</strong><br>
                        <span>${precio} · ${distTexto} km desde nuestro depósito</span>
                    </div>
                </div>`;
        } else {
            resultado.innerHTML = `
                <div class="resultado-zona">
                    <span class="resultado-emoji">❌</span>
                    <div>
                        <strong>Fuera de zona (${distTexto} km)</strong><br>
                        <span>Por ahora no llegamos hasta ahí. ¡Seguimos creciendo!</span>
                    </div>
                </div>`;
        }
        resultado.className = `cobertura-resultado ${clase}`;
        resultado.style.display = 'block';

        // Marcar en el mapa si está cargado
        if (_coberturaMap) {
            if (_marcadorCliente) _marcadorCliente.remove();
            _marcadorCliente = L.marker([clienteLat, clienteLng])
                .addTo(_coberturaMap)
                .bindPopup(`📍 ${display_name.split(',')[0]}<br>${zona ? zona + ' — ' + precio : 'Fuera de zona'}`)
                .openPopup();
            _coberturaMap.setView([clienteLat, clienteLng], 12);
        }
...
function guardarZonasEnvio() {
    zonasEnvio = {
        zona1: {
            precio: parseFloat(document.getElementById('zona1Precio')?.value) || 0,
            texto:  document.getElementById('zona1Texto')?.value || 'Envío gratis'
        },
        zona2: {
            precio: parseFloat(document.getElementById('zona2Precio')?.value) || 500,
            texto:  document.getElementById('zona2Texto')?.value || 'Envío $500'
        },
        zona3: {
            precio: parseFloat(document.getElementById('zona3Precio')?.value) || 1000,
            texto:  document.getElementById('zona3Texto')?.value || 'Envío $1.000'
        },
    };
    guardar();
    // Resetear mapa para que tome los nuevos textos
    if (_coberturaMap) { _coberturaMap.remove(); _coberturaMap = null; }
    toast('✅ Zonas de envío guardadas', 'success');
}

function cargarUIZonas() {
    const z = zonasEnvio;
    const set = (id, val) => { const el = document.getElementById(id); if (el) el.value = val; };
    set('zona1Precio', z.zona1.precio);
    set('zona1Texto',  z.zona1.texto);
    set('zona2Precio', z.zona2.precio);
    set('zona2Texto',  z.zona2.texto);
    set('zona3Precio', z.zona3.precio);
    set('zona3Texto',  z.zona3.texto);
}
...
function filtrarProductos() {
    ...
    // Filtro del menú de secciones rápidas
    const sm = SECCIONES_MENU[seccionActiva];
    if (sm && sm.seccion) {
        prods = prods.filter(p => p.seccion === sm.seccion);
        if (sm.familias) {
            prods = prods.filter(p => sm.familias.includes(p.familia));
        }
    }
    ...
    // Filtro favoritos
    if (ordenActual === 'favoritos') {
        prods = prods.filter(p => favoritos.includes(p.id));
    }

    // Ordenar
    if (ordenActual === 'precio_asc') {
        prods.sort((a, b) => calcularPrecio(a, 1) - calcularPrecio(b, 1));
    } else if (ordenActual === 'precio_desc') {
        prods.sort((a, b) => calcularPrecio(b, 1) - calcularPrecio(a, 1));
    } else if (ordenActual === 'nombre_asc') {
        prods.sort((a, b) => a.nombre.localeCompare(b.nombre, 'es'));
    }
    // 'vendidos' y 'favoritos' ya están ordenados por venta_diaria del sort anterior
...
function actualizarFiltros() {
    // Los filtros de sección ahora los maneja el menú de secciones rápidas
    // Esta función se conserva para compatibilidad pero ya no actualiza selects
}
...
function actualizarContadorFiltros() {
    // Manejado por el menú de secciones — no hay badge ni botón reset viejo
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Filtros por sección manejados por el menú de secciones rápidas

    document.getElementById('lastUpdate').textContent = new Date().toLocaleDateString('es-AR');

    // Listeners del menú de secciones
    document.querySelectorAll('.seccion-btn').forEach(btn => {
